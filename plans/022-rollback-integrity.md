...
> reviewer).
>
> **Drift check (run first)**: `git diff --stat e80b564..HEAD -- bin/omocachy-profile-import.sh`
...
**In scope**: `bin/omocachy-profile-import.sh` only.
...
**Verify**: `grep -n 'restored.tsv' bin/omocachy-profile-import.sh` → every
...
**Verify**: `bash -n bin/omocachy-profile-import.sh` → exit 0;
`grep -n 'write_rollback' bin/omocachy-profile-import.sh` → definition + one
...
**Verify**: `grep -n '\-e "$HOME/$rel" || -L' bin/omocachy-profile-import.sh` →
one match.
...
EOF
HOME="$h" bin/omocachy-profile-import.sh --bundle "$d/bundle" --only configs --yes
ls "$h/.local/state/omocachy/backups/"*/rollback.sh
"$h/.local/state/omocachy/backups/"*/rollback.sh --dry-run
```
...
- [ ] No files outside `bin/omocachy-profile-import.sh` are modified
...
- The generated rollback now depends on `restored.tsv` staying beside it;
  `bin/omocachy-profile-export.sh` is unrelated, but any future "move the
