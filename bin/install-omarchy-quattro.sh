...
ensure_hookdir_lines() {
    ...
    if grep -qxF "HookDir = $OMACACHY_HOOK_DIR/" "$conf" 2>/dev/null; then
    ...
    if grep -qE '^\s*HookDir\s*=' "$conf" 2>/dev/null; then
        ...
        echo "Inserting \"HookDir = $OMACACHY_HOOK_DIR/\" after the last existing HookDir line in /etc/pacman.conf."
        if $DRY_RUN; then
            echo "DRYRUN: rewrite /etc/pacman.conf with that one line added"
        else
            tmp="$(mktemp)"
            awk -v ours="HookDir = $OMACACHY_HOOK_DIR/" '
    ...
    if ! $DRY_RUN && ! grep -qxF "HookDir = $OMACACHY_HOOK_DIR/" /etc/pacman.conf; then
        echo "Error: could not register $OMACACHY_HOOK_DIR in /etc/pacman.conf (no [options] section?). Refusing to continue: the limine pacman hooks would run on a $BOOTLOADER machine." >&2
        exit 1
    fi
}
