...
if [[ -n $QS ]]; then
    pass "$QS $(pacman -Q "$QS" | awk '{print $2}') installed (the shell runtime)"
...
if [[ -d $PLUGIN_DIR ]]; then
    total=$(find "$PLUGIN_DIR" -mindepth 1 -maxdepth 1 -type d | wc -l | tr -d ' ')
...
# Hyprland: only meaningful inside a session.
if [[ -n ${HYPRLAND_INSTANCE_SIGNATURE:-} ]] && command -v hyprctl &>/dev/null; then
    ...
    if omarchy-shell -q shell ping &>/dev/null || omarchy-shell shell ping &>/dev/null; then
    ...
    skip "no Hyprland session in this context — hyprctl/omarchy-shell checks"
fi
