...
if $SLIM; then
    printf '%s\n' \
        '.config/omarchy/themes/*/backgrounds' \
        '.local/share/omarchy/themes/*/backgrounds' >>"$EXCLUDE_FILE"
fi
...
# ---------------------------------------------------------------------------
# Secret sweep: names that look like credentials leave the payload. Every hit
# is reported, so a false positive (a script called *-token-fetch) is visible
...
if ! $DRY_RUN; then
    ...
    while IFS= read -r hit; do
        printf '%s\n' "${hit#"$BUNDLE/home/"}" >>"$SECRETS_REMOVED"
        rm -f "$hit"
...
if ! $DRY_RUN; then
    ...
    cp -a "$SECRETS_REMOVED" "$INLINE_SECRETS" "$BUNDLE/system/"
